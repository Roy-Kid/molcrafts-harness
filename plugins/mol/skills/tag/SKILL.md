---
name: tag
description: Push an existing local release tag to upstream so the release workflow fires. Auto after /mol:release has merged the version-bump commit. No approval wait when safety checks pass. Refuses orphan tags and force overwrites.
argument-hint: "[<tag>]"
---
...
- **Never** create tags here (creation is `/mol:release`).
